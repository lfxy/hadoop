...
public class DatanodeManager {
  ...
  public DatanodeCommand[] handleHeartbeat(DatanodeRegistration nodeReg,
    ...
    if (totalBlocks > 0) {
      ...
      LOG.debug("Pending replication tasks: {} ec replicated tasks: {} ec reconstruction tasks: {}.",
          numReplicationTasks, numEcReplicatedTasks, numECReconstructedTasks);
      // check pending replication tasks
      List<BlockTargetPair> pendingList = nodeinfo.getReplicationCommand(
          numReplicationTasks);
      pendingList.addAll(nodeinfo.getECReplicatedCommand(numEcReplicatedTasks));
